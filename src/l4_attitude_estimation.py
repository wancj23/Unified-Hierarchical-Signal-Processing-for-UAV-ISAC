...
class L4_AttitudeEstimator:
    ...
    def __init__(self, arm_length_L: float = 0.5, dt: float = 0.001):
        """
        ...
        # Reliability fusion weights \mathbf{w}_4 = [w_f, w_s]^T
    ...
    def _estimate_robust_rotor_speed(self, v_k_t: np.ndarray) -> float:
        ...
        adaptive_distance = max(1, int(0.05 / self.dt))  # Min 50ms interval assumption
        peaks, _ = find_peaks(np.abs(v_k_t), distance=adaptive_distance, prominence=0.5)
    ...
    def _invert_spatial_attitude(self, omegas: np.ndarray) -> Tuple[float, float, float]:
        ...
        delta_z = np.sum(thrust_forces) * 1e-4  # Scaled aggregate thrust metric
        ...
        delta_a_z = np.sign(delta_a_z) * max(abs(delta_a_z), 1e-6)
        ...
        # Using normalized pseudo-angles derived from relative RPMs
        pseudo_thetas = (omegas / np.max(omegas)) * np.pi
...
if __name__ == "__main__":
    ...
    time_steps = np.linspace(0, 1.0, 1000)  # 1 second, dt=1ms

    # 4 distinct rotor RPMs (in Hz: 50, 52, 48, 55) -> corresponding to rad/s
    simulated_frequencies = [50.0, 52.0, 48.0, 55.0]
    ...
    for i, freq in enumerate(simulated_frequencies):
        ...
        v_k_t = 20.0 * np.sin(2 * np.pi * freq * time_steps) + np.random.normal(0, 2.0, 1000)
