...
class L2_TargetClassifier:
    ...
    def __init__(self, v_max: float, delta_v: float = 2.0, gamma_uav: float = 0.65):
        ...
        # Frozen separating hyperplane fitted on the training partition and
        # selected before test evaluation (UAV versus non-UAV targets).
...
if __name__ == "__main__":
    ...
    velocity = np.full(100, 49.0)

    mock_kinematics = {
        'velocity': velocity,
        'range': np.linspace(100, 105, 100) + rng.normal(0, 0.02, 100)
    }

    # Initialize L2 Classifier with a v_max corresponding to M=16 (e.g., 50.0 m/s)
    classifier_l2 = L2_TargetClassifier(v_max=50.0, delta_v=2.0, gamma_uav=0.65)
